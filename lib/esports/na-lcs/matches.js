...
function formatMatch(data, game) {
	console.log("adding match " + game.matchId + ":" + game.gameId + "...");
	...
	var dbMatch = new Match(matchToAdd).save();
...
exports.get = function() {
	data.forEach(function(match) {
		Match.findOne({ matchId: match.matchId, gameId: match.gameId }, function(err, found) {
			if(err) console.error(err)
			if(found) console.log("Match " + match.matchId + ":" + match.gameId + " found!");
			else {
				console.log("Match " + match.matchId + ":" + match.gameId + " was not found in the database. Adding...");
				acs.getMatch(data[0].statUrl, function(res) {
					formatMatch(res, data[0]);
				});
			}
		});
	});
