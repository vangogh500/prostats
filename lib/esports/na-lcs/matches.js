...
function formatMatch(data, game) {
	Match.findOne({ matchId: game.matchId, gameId: game.gameId }, function(err, found) {
		...
		Team.findOneAndUpdate({ acronym: game.blue.acronym, matches: { $ne: dbMatch._id }}, { $push: { matches: dbMatch._id }}, function(err, updated) {
			if(err) console.error(err);
		...
		Team.findOneAndUpdate({ acronym: game.red.acronym, matches: { $ne: dbMatch._id }}, { $push: { matches: dbMatch._id }}, function(err, updated) {
			if(err) console.error(err);
			if(updated) console.log("\t added match to team " + game.blue.acronym);
			else console.log("\t WARNING: Could not find team. Make sure to get teams first");
		});
		data.participantIdentities.forEach(function(participant) {
			...
			playerName = playerName.replace(playerName[0], playerName[0].toUpperCase());
			Player.findOneAndUpdate({ name: playerName, matches: { $ne: dbMatch._id }}, { $push: { matches: dbMatch._id }}, function(err, updated) {
				if(err) console.log(err);
				if(updated) console.log("\t added match to player " + playerName);
				else console.log("\t WARNING: Could not find player " + playerName + ". Make sure to get players first");
			});
		});
	});
}
