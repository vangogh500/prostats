...
function scrapePlayerInfo(link, cb) {
	crawler.get('http://na.lolesports.com' + link, function($player) {
		...
		console.log('\t player imgURL:' + $player('.player-profile-background img').attr('src'));
		console.log('\t player bio:' + $player('.field-type-text-with-summary p').text());
