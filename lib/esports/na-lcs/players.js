...
exports.formatPlayer = function(player, teamName) {
	...
	Player.findOne({ name: player.name }, function(err, found) {
		...
		if(found) {
			...
			Team.findOne({ name: teamName, roster: { $ne: found._id }}, function(err, found) {
				if(found) console.log("SUCCESS " + found.name);
			});
			\\ Team.findOneAndUpdate({ name: teamName, roster: { $ne: found._id }}, { $push: { roster: found._id }});
		}
		else {
