...
exports.formatPlayer = function(player, teamName) {
	...
	Player.findOne({ name: player.name }, function(err, found) {
		...
		if(found) {
			...
			Team.findOneAndUpdate({ name: teamName, roster: { $ne: found._id }}, { $push: { "roster": found._id }}, function(err){
			...
			var toAdd = new Player({
				...
				Team.findOneAndUpdate({ name: teamName, roster: { $ne: toAdd._id }}, { $push: { "roster": toAdd._id }}, function(err){
					if(error) console.error(err);
				});
			});
		}
