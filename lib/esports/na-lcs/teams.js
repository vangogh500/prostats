...
function scrapeTeamInfo(teamLink, teamName) {
	crawler.get('http://na.lolesports.com' + teamLink, function($team) {
		...
		$team('.team--extra-info').each(function() {
			if($team('span', '.team--extra-info').eq(0).html() == "Nationality:") 
				nationality = $team('span', '.team--extra-info').eq(1).html();
			else if($team('span','.team--extra-info').eq(0).html() == "Founded:")
				date = new Date($team('span', '.team--extra-info').eq(1).html());
		});
