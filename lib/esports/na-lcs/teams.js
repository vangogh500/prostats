...
function scrapeTeamInfo(teamLink, teamName) {
	crawler.get('http://na.lolesports.com' + teamLink, function($team) {
		...
		$team('.team--extra-info').each(function() {
			console.log($team(this).find('span').eq(1).html());
			if($team(this).find('span').eq(0).html() == "Nationality:") 
				nationality = $team(this).find('span').eq(1).html();
			else if($team(this).find('span').eq(0).html() == "Founded:")
