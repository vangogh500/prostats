...
function scrapeTeamInfo(teamLink, teamName) {
	crawler.get('http://na.lolesports.com' + teamLink, function($team) {
		...
		$team('.team--extra-info').each(function() {
			console.log($team('span', this).eq(0).html());
			if($team('span', this).eq(0).html() == "Nationality:") 
				nationality = $team('span', this).eq(1).html();
			else if($team('span',this).eq(0).html() == "Founded:")
				date = new Date($team('span', this).eq(1).html());
		});
