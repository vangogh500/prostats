...
function formatTeam(team) {
	...
	Team.findOne({ name: team.name }, function(err, found) {
		...
		for(var player in team.roster) {
			players.formatPlayer(team.roster[player], function(dbPlayer) {
				console.log("\t \t Searching if player exists in team...");
				if(dbTeam.roster && dbTeam.roster.indexOf(dbPlayer._id) > -1) console.log("\t \t \t Found!");
				else {
					console.log("\t \t \t Was not found to be on the team's roster. Adding...");
					// some pointer trickery
					var tempArr = dbTeam.roster;
					dbTeam.roster = tempArr.push(dbPlayer._id);
					console.log(dbTeam.roster);
					dbTeam.save();
				}
			});			
		}
